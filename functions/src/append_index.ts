...
export const analyzeMeal = functions.https.onCall(async (data, context) => {
    // Verificar autenticação
    if (!context.auth) {
        throw new functions.https.HttpsError(
            'unauthenticated',
            'O usuário deve estar autenticado para analisar refeições.'
        );
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new functions.https.HttpsError(
                'internal',
                `Erro no serviço de análise: ${response.status}`
            );
        }
        ...
        try {
            ...
            throw new functions.https.HttpsError(
                'internal',
                'A resposta do serviço de análise não é um JSON válido.'
            );
        }
        ...
        throw new functions.https.HttpsError(
            'internal',
            'Falha ao conectar com serviço de análise.'
        );
    }
});
